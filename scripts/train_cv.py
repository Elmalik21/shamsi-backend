...
def main():
    ...
    results = model.train(
        data=data_yaml_path,
        epochs=150,          # تم رفعها لـ 150 كما طلبت ليتعلم الأنماط جيداً
        patience=25,         # الإيقاف المبكر إذا لم يتحسن لـ 25 دورة
        imgsz=800,           # دقة أعلى ليرى البكسلات الصغيرة بوضوح
        batch=8,             # تقليل الـ Batch قليلاً لتفادي امتلاء الذاكرة مع الدقة العالية
